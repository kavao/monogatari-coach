...
description: >-
  ...
  manga/_assets/<manga_XX>/backgrounds/（背景資料）、illustrations/_assets/<illustration_XX>/（挿絵・表紙）のディレクトリを
...
`.rulesync/rules/workflow-specification.md` の **「画像保存先」** と Tag Mode / Manga Tag Mode の詳細仕様に合わせ、**フォルダだけ先に機械的に用意**する。

- **タグ**: `tag/kazuki.md` と同名の **`tag/kazuki/`** に、そのキャラの生成画像をすべて保存する。
- **漫画（コマ・ページ）**: `manga/manga_01.md` に対応する **`manga/_assets/manga_01/comic/`** に保存する。**既定運用では `comic/` 直下**を使う。ストックの分類は**章（`manga_XX`）まで**で足りる。
...
**フォルダ作成**（`tag/*.md` があれば `tag/<stem>/`、`manga/manga_*.md` があれば `manga/_assets/<stem>/comic/` と `backgrounds/`、`illustrations/pages/illustration_XX_pYY.yaml` があれば `illustrations/_assets/illustration_XX/`）:
...
- **`image_provider_generate`** の `output_dir` に、上記の **`tag/<romaji>`**、**`manga/_assets/<manga_XX>/comic`**、または **`illustrations/_assets/<illustration_XX>`** を指定する。`comic/k03` などは手動でコマ別に分けたい場合だけ使う。
...
- **`manga/manga_*.md` の各コマ**（`## Page` → `## step1` 内の `tag:` … `和訳:`）を `manga/_assets/<manga_XX>/comic/` に出す場合は `tools/image_provider_novel_manga_batch.py <作品フォルダ>`。**既定では `comic/` 直下に保存され、`k01` などは使わない。** ファイル名接頭辞は `manga_01_p02_k03`（ページ・コマ番号）。
...
- 規約: `.rulesync/rules/workflow-specification.md`（画像保存先）
- 詳細仕様: `.rulesync/rules/workflow-specification.md`（Tag Mode / Manga Tag Mode）
