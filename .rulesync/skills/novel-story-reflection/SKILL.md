...
- **実文字数・状態の更新**: `design_specification.md` の「執筆スケジュール」等の表に「実文字数」カラムや「状態」カラムが存在する場合、`python tools/novel_char_count.py` の最新計測結果に基づき、該当章の実文字数（例: `8,051字`）、状態（`未着手` → `初稿完了` / `清書完了` 等）、および全体の合計実文字数を必ず記入・更新する（空欄のまま放置しない）。
