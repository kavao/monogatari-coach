...
- **画像内文字**: 原則なし（表紙絵にタイトルを焼かない）
- **題字方針**: （`組版` / `logo_asset` / `後回し` — `_meta.md` §3.1 と一致させる）
- **後載せ**:
  - `組版`: `cover.yaml` の `layers[id=title]` を `type: text`
  - `logo_asset`: `cover/title_logo_plan.md` → 採用PNG → `type: logo_asset`（雛形: `_how_to.example/publishing/`）
- **備考**: <!-- 要る場合のみ。例: 上部30%をタイトル安全圏として空ける。著者名は別レイヤー。 -->
