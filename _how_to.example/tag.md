...
- **固定に入れない**: 状況で変わる衣装、`standing` 等の姿勢、背景、**露出・性器・裸限定タグ**、**`solo`（人数固定。ソロ資料バリアントへ）**、**キャラ名・人物名トークン**（`name` / `name_en` / `character_id` は YAML メタのみ）。
...
| 見出し | `## 000. 固定基礎` |
| `variant_id` | **`000_base`** |
| 入れる | 性別（`1girl` / `1boy` 等）・体格・髪・目・肌・種族・固定小物 |
| 入れない | **`solo`**（継承用 `000_base` には載せず、ソロ資料バリアントのみ）、**キャラ名・人物名トークン**（`character_id` / `name_en` / `name` はメタ情報のみ）、衣装、姿勢、背景、表情、nsfw、局部・行為、**`distinctive_features` への裸限定タグ** |
...
- **Danbooru Tags** には外見・衣装・ポーズ・状況タグのみ。**キャラ名トークンは入れない**（識別は YAML の `character_id` / `name` / `name_en` と Markdown 見出し）。
- **Caption** には英語のキャラ名を入れてよい（人間向け説明）。
