...
def test_apply_replaces_avoid_with_use() -> None:
    rules = [
        ("長いフレーズABC", "短い"),
        ("殴りかかる", "腕を振りかぶる構図"),
    ]
    assert apply_step2_paraphrase("人物が殴りかかる", rules=rules) == "人物が腕を振りかぶる構図"
    assert (
        apply_step2_paraphrase("長いフレーズABCと短い", rules=rules)
        == "短いと短い"
    )


def test_longer_avoid_applied_first() -> None:
    """長い avoid を先に適用するため、aaaa は aaa→Y のあと aa は残らない。"""
    rules = [("aa", "X"), ("aaa", "Y")]
    assert apply_step2_paraphrase("aaaa", rules=rules) == "Ya"
...
def test_default_yaml_loads() -> None:
    ...
    assert "上に乗っている" in out
    assert "跨っている" not in out
