...
[`novel_status.py`](../tools/index.md) の `--next` で、ファイルシステムの状態から次モード（Plan / Tag / Writing 等）を1行表示します。
...
画像生成向けのポーション・タグ前後挿入・`workflows` は `_meta.yaml` に記述します。読み込みは **`tools/novel_meta_yaml.py`（内部モジュール）** が担い、各バッチ・`novel_status.py` から import されます。ユーザーが直接叩く CLI ではありません。雛形は `_how_to.example/_meta.yaml.example` を参照してください。
