...
次の selected 必須は、**新規企画の Gate B 完了**と、ユーザーが **明示した Gate B 再実施**だけに適用します。既存作品の旧形式 `_meta.md`（`読んだ _how_to` のパス箇条書きなど）は未変換のまま **grandfathered** とし、この selected 必須では不合格にしません。旧形式では索引以外の葉パスと非該当理由があれば足ります。

- 作品経路と作品プロファイルが明示されている（該当なしは理由付き）
- （新規または再実施）作業用 `_how_to/_index.md` があればそれを選定入口とし、標準 `_how_to.example/_index.md` を発見用に併読する。作業用に入口が無い標準の新葉は selected にしない。作業用が無ければ標準だけから選ぶ。選んだ **葉**が `_meta.md` の selected にあるか、任意の `pack_id` 展開後の葉として検査できる。パックは指示のない作品には付けない。索引は選定補助
- not_applicable は、プロファイル上の必須候補の見送り、または `genre/*` のようなグループに限る（カタログ全未選択は不合格の材料にしない）
...
- 完了報告に、新形式なら selected の要約、旧形式なら読んだ葉の要約が含まれる

Writing Mode の到達テスト（「まだ書かないで」）では、契約再読の実施は求めません。本文を書くターンの完了条件はスキル **`novel-text-file-output`** です。
...
Cursor がプロジェクト準備と本文出力の手順を選び、本文正本を `novels/.../_novel_text/` とし、チャットだけで完了扱いにしなければ合格です。METRON / CHRONOS が ON の作品では writing_bridge と `_novel_text` 直接更新を同じ場面で重ねない説明があればなお良いです。
